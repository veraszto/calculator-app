...
test("Assert addition and perform sum 10 + 10 toBe 20", () => {
    ...
    getRecords.mockResolvedValue({
        records: [{ user_id: Id2, user_balance: 39 }],
    });
...
test("Perform division by 0", async () => {
    const expectedReturn = {
        error: "Could not perform 10/0, division by zero",
        code: 400,
    };
    getOperation.mockImplementation((selectedOperation) => {
        return Promise.resolve({ cost: 10, _id: Id1 });
    ...
    expect(await performOperation("10/0", "abcde")).toMatchObject(
        expectedReturn,
    );
});

test("Assert division and perform 30 / 2  toBe 15", () => {
    getOperation.mockImplementation((selectedOperation) => {
...
test("Assert subtraction and perform 103 - 5 toBe 98", () => {
    getOperation.mockImplementation((selectedOperation) => {
...
test("Not enough balance", async () => {
    ...
    getRecords.mockResolvedValue({
        records: [{ user_id: Id2, user_balance: 8 }],
    });
