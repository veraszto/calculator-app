...
async function performOperation(string, userId) {
    ...
    if (selectedOperation === null) {
        return {
            error: `${string} has not fit to any of the operations`,
            code: 400,
        };
    }
    ...
    return getOperation(selectedOperation).then((operation) => {
        ...
        return getRecords(userId, 0, 1).then(async (latestRecord) => {
            ...
            if (selectedOperation === "random_string") {
                try {
                    randomString = await fetch(StringGeneratorUrl).then((res) =>
                        res.text(),
                    );
                    console.log("/operation", randomString.toString());
                } catch (error) {
                    return {
                        error: "Could not fetch from random.org",
                        code: 401,
                    };
                }
            }
            ...
            if (selectedOperation === "square_root") {
                let result;
                const errorResponse = {
                    error: `Could not perform square root from ${string} `,
                    code: 400,
                };
                try {
            ...
            try {
                ...
                case "division":
                    if (numbers[1] === 0) {
                        return {
                            error: `Could not perform ${string}, division by zero`,
                            code: 400,
                        };
                    }
                    result = numbers[0] / numbers[1];
